...
function getAddToEntries(srcRoot: string, ignores: RegExp[], entries: EntryType, resolve?: (...args: any[]) => any) {
  let moduleCount = 0;
        ...
        fsx
          ...
          .then((fileJson: ComponentJson) => {
            ...
            const comps = Object.values(fileJson.usingComponents || {}).map((pathname: string) => {
              return isRelativePath(pathname) ? path.join(dirname, pathname) : pathname.replace(/^\//, '');
            });
            addToEntries(comps);
            count(-1);
          })
          .catch(() => {
            count(-1);
            console.log(
              chalk.yellow(`Component not found: ${path.dirname(fileBasename)}, used by ${chalk.green(from)}\n`),
            );
          });
        return;
      }
...
function _getEntries(srcRoot: string, ignores: string[] = [], sync?: boolean) {
  const appJson: AppJsonType = fsx.readJSONSync(path.join(srcRoot, 'app.json'));
  ...
  const appUsingComponents = Object.values(appJson.usingComponents || {})
    ...
    .map((comp) => {
      return isRelativePath(comp) ? comp : comp.replace(/^\//, '');
    });
 ...
 */
export const getEntries = (srcRoot: string, ignores: string[] = []): Promise<EntryType> => {
  return _getEntries(srcRoot, ignores) as Promise<EntryType>;
};

export const getEntriesSync = (srcRoot: string, ignores: string[] = []): EntryType => {
...
export const getEntriesAndUnusedPaths = (srcRoot: string, ignores: string[] = []): GetEntriesAndUnusedPathsReturn => {

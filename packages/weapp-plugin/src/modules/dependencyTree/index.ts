...
export class DependencyTree extends DependencyTreeNode {
  ...
  private addAppChunk(appJson: IWeappAppConfig) {
    ...
    this.addAllAssets();
    /** 添加主包里的 pages */
    this.addAllToModule(APP_CHUNK_NAME, pages);
    /** 添加主包使用的 components */
    this.addAllToModule(APP_CHUNK_NAME, Object.values(usingComponents));
    /** 添加 TabBar */
  ...
  private addTabBar(tabBar: IWeappAppConfig['tabBar']) {
    ...
    if (custom) {
      ...
      this.addModule(APP_CHUNK_NAME, tabBarEntryPath);
    }
    ...
    this.addAllToModule(APP_CHUNK_NAME, assets);
  }
  ...
  private addSubPackageChunk(appJson: IWeappAppConfig) {
    ...
    subPackages.map((subPackage) => {
      ...
      return this.addAllToModule(independent ? chunkName : APP_CHUNK_NAME, pages, resolve);
    });
  }
   ...
   * @param resolve
   */
  private addAllToModule(chunkName: string, resources: string[], resolve = this.resolve) {
